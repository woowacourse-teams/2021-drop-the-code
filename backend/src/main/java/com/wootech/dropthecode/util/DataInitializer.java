...
public class DataInitializer {
    ...
    private List<TeacherProfile> insertTeacherProfile(long i, Map<Long, Member> memberMap) {
        List<TeacherProfile> teacherProfiles = Arrays.asList(
                new TeacherProfile("배민 출신 백엔드 개발자", "배민 1타 강사. 열심히 가르쳐드리겠습니다.", 5, 2, 1.3, memberMap.get(1L + i * 6)),
                new TeacherProfile("쿠팡 이츠 출신 백엔드 개발자", "빡세게 가르쳐드리겠습니다.", 4, 5, 1.2, memberMap.get(2L + i * 6)),
                new TeacherProfile("우테코 갓 졸업생", "열심히 하겠습니다.", 10, 10, 5.3, memberMap.get(3L + i * 6)),
                new TeacherProfile("배민 인프라 담당자", "화이팅", 2, 1, 10.1, memberMap.get(4L + i * 6)),
                new TeacherProfile("Amazon 서버 개발자", "실리콘밸리 출신 인프라 개발자. 영어로 수업합니다.", 7, 3, 2.9, memberMap.get(5L + i * 6)),
                new TeacherProfile("배민 프론트 개발자", "화이팅", 6, 1, 8.4, memberMap.get(6L + i * 6))
        );
